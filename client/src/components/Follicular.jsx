...
export default function Follicular() {
    ...
    const addWorkout = async () => {
      try {
        const response = await fetch("/api/follicular", {
          method: "POST",
          headers: {
    ...
    const deleteWorkout = async (id) => {
        try {
          const response = await fetch(`/api/follicular/${id}`, {
           method: "DELETE", 
          });
          const data = await response.json();
          setAllJams(data);
    ...
    return (
        ...
        <div className="container pt-4">
          <h1 className="text-bold">Follicular Phase</h1>
          <br />
...
{/*       
      ...
      <Link to='/homepage' >Back</Link></div>
      </div>
  )
}
